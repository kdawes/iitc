// ==UserScript==
// @id             iitc-plugin-show-portal-weakness v2
// @name           iitc: show portal weakness
// @version        0.6.02
// @namespace      https://github.com/breunigs/ingress-intel-total-conversion
// @updateURL      https://raw.github.com/shottrue/iitc/gh-pages/plugins/show-portal-weakness.user.js
// @downloadURL    https://raw.github.com/shottrue/iitc/gh-pages/plugins/show-portal-weakness.user.js
...
window.plugin.portalWeakness.portalAdded = function(data) {
  ...
  if(getTeam(d) !== 0) {
    ...
    if(window.getTotalPortalEnergy(d) > 0 && window.getCurrentPortalEnergy(d) < window.getTotalPortalEnergy(d)) {
      ...
      only_shields = false;
      //console.log( "step 1. portal_weakness: " + portal_weakness + "; energy:" + window.getCurrentPortalEnergy(d)/window.getTotalPortalEnergy(d) );
    }
    ...
    $.each(d.resonatorArray.resonators, function(ind, reso) {
      if(reso === null) {
        ...
        resCount++;
        if(reso.level >= resIsHL) {
            resCountHL++;
            // If has resonators L8 make the portal bigger
            if (reso.level > 7) {
                resRaduisHL++;
            }
        }
